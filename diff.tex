...
\documentclass[aip, jcp, preprint, linenumbers]{revtex4-1}
%DIF LATEXDIFF DIFFERENCE FILE
%DIF DEL orig.tex   Fri Nov 18 11:05:12 2016
%DIF ADD main.tex   Fri Nov 18 11:04:44 2016
...
%DIF 31a31
\renewcommand{\algorithmicensure}{\textbf{Let:}} %DIF > 
%DIF -------
...
\DIFaddbegin \author{\DIFadd{Brooke E. Husic}}
%DIF > \email{bhusic@stanford.edu}
\DIFaddend \author{Vijay S. Pande}
...
\DIFdelbegin \DIFdel{Later on, we will show how the formulation also extends naturally to multiple orthogonal reaction coordinates.
}\DIFdelend %DIF > Later on, we will show how the formulation also extends naturally to multiple orthogonal reaction coordinates.
...
Now, consider the task of constructing an approximation to $p_t$. This approximation, $\tilde{p}_t$, is constrained to depend only on $\mu(x)$, \DIFdelbegin \DIFdel{$q(x)$, }\DIFdelend $\tau$, $t$, \DIFdelbegin \DIFdel{and }\DIFdelend $\langle q | p_0 \rangle$\DIFaddbegin \DIFadd{, $q(x)$, and the equilibrium mean and variance of $q(x)$}\DIFaddend . That is, given knowledge of the equilibrium distribution, the \emph{ansatz} reaction coordinate, its timescale, and \emph{no other} information about the current ensemble, $p_0$, beyond its projection onto the \emph{ansatz} reaction coordinate, our goal is to construct a prediction of the future ensemble at some later time $t$.

%DIF >  and unit variance
A basic dimensional analysis argument and the constraint that $\int_\Omega \tilde{p}_t=1$ is sufficient to establish that, assuming that $q$ is measured in a system of units such that it has mean zero \DIFdelbegin \DIFdel{and unit variance }\DIFdelend in the equilibrium ensemble, the functional form of $\tilde{p}_t$ given $q$ must be
\begin{align}
\label{eq:functionalform}
\tilde{p}_t(x) = \mu(x) + f(t/\tau) \DIFdelbegin %DIFDELCMD < \langle %%%
\DIFdel{q| p_0 }%DIFDELCMD < \rangle %%%
\DIFdel{q(x) }\DIFdelend \DIFaddbegin \DIFadd{\frac{\langle q| p_0 \rangle (q - \langle \mu| q \rangle)}{\langle \mu | q^2 \rangle} }\DIFaddend \mu(x),
%DIF > \tilde{p}_t(x) = \mu(x) + f(t/\tau,\langle q| p_0 \rangle) q(x) \mu(x),
\end{align}
\DIFdelbegin \DIFdel{for }\DIFdelend \DIFaddbegin \DIFadd{where $f$ is }\DIFaddend some non-random function \DIFdelbegin \DIFdel{, $f$, }\DIFdelend that is independent of $x$ \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{and $\langle q^2| p_0 \rangle$ is the variance of $q$.
%DIF > {\color{red}Later on, we will see that this choice is motivated by the spectral expansion of the integral operator associated with the ensemble dynamics and show that $f(t/\tau)=e^{-t/\tau}$, i.e.~the autocorrelation function of the \textit{ansatz} reaction coordinate $q$.}
}\DIFaddend Later on, we will show that \DIFdelbegin \DIFdel{$f$ }\DIFdelend \DIFaddbegin \DIFadd{$\tilde{p}_t(x)$ }\DIFaddend is necessarily an exponential, $f(t/\tau)=e^{-t/\tau}$.
...
By design, these algorithms rest on the pre-identification of the $A$ and $B$ states \DIFdelbegin \DIFdel{, and }\DIFdelend are not naturally suited to systems with more than two metastable states, although multiple-state extensions are available.\cite{rogal2008multiple} 
\DIFdelbegin \DIFdel{For the }\DIFdelend \DIFaddbegin \DIFadd{When these two states are both known }\textit{\DIFadd{a priori}} \DIFadd{and metastable, then we expect, but have not proven, that the committor function and the natural reaction coordinate are nearly equivalent. Algorithms that leverage this }\textit{\DIFadd{a priori}} \DIFadd{knowledge have the advantage of requiring significantly less sampling to converge their reaction coordinate estimators.
However, for the }\DIFaddend reasons discussed in \cref{sect:uniquely_dertmined}, we \DIFdelbegin \DIFdel{thus }\DIFdelend dispute the claim that the committor should be taken as the \emph{perfect} or \emph{exact} reaction coordinate.\cite{grunwald2009transition, ma2005automatic, peters2006obtaining, krivov2011numerical} The authors' experiences with large-scale simulations of protein folding and activation on Folding@Home have shown that it can be difficult to locate and precisely define these metastable states. This suggests that, for an important class of problems, the metastable states should be constructed from the output of some model, as opposed to being treated as a modelling input.\cite{shirts2000screen, voelz2010molecular, shukla2014activation} These considerations motivate our formulation of the natural reaction coordinate in a manner independent of the choice to label certain regions of phase space as $A$ or $B$.

\DIFaddbegin \DIFadd{We note that others have also defined a reaction coordinate consistent with the intuitive mathematical properties specified in Section~\ref{subsect:Maximally_predictive}, such as the subset of leading eigenfunctions estimated by diffusion maps.\mbox{%DIFAUXCMD
\cite{nadler2006diffusion, rohrdanz2011determination, Boninsegna_JCTC15}
}%DIFAUXCMD
...
%DIF >  diffusion maps, MSMs, tICA, sparse tICA are all estimators for the same object. ours is better b/c direct kinetic info and sparsity


\DIFaddend % For example, \citet{ma2005automatic} harvested conformations on reactive trajectories with TPS, measured approximate committor values by shooting trajectories from each conformation, and trained a neural network to predict these values using structural order parameters. More efficient methods using likelihood maximization have been developed.
...
\DIFdelbegin \DIFdel{We note that others have also used the term reaction coordinate for these eigenfunctions.\mbox{%DIFAUXCMD
\cite{nadler2006diffusion, rohrdanz2011determination}
}%DIFAUXCMD
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Preliminaries}
...
\subsection{\DIFdelbegin \DIFdel{Analysis of the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend error functional}
...
We now apply this spectral decomposition of the transfer operator to the analysis of the error functional from \cref{subsect:Maximally_predictive} and \DIFdelbegin \DIFdel{prove why }\DIFdelend \DIFaddbegin \DIFadd{show that }\DIFaddend the natural reaction coordinate is equal to the second transfer operator eigenfunction, $q^* = \psi_2$.
...
\DIFaddbegin \noindent{}\DIFaddend The approximate transfer operator, $\tilde{\mathcal{T}}(t)$, is rank 2; it has two non-zero eigenvalues, $1$ and $f(t/\tau)$, with associated eigenfunctions $1$ and $q(x)$ respectively.
...
\DIFaddbegin \label{eq:error_functional}
\DIFaddend E[q] &= \max_{u_0} || (\mathcal{T}(t) - \tilde{\mathcal{T}}(t)) \circ u_0 ||_\mu^2,
\end{align}
where the maximum is understood to be taken over properly normalized $u_0$, $||u_0||_\mu = 1$. 

\DIFdelbegin \DIFdel{Now, we prove by why $\psi_2 = \min_q E[q]$. Observe that for any $q$, there exists a function $v(x)$ in the span of the first three eigenfunctions of $\mathcal{T}$, $v=a_1 \psi_1+ a_2 \psi_2 + a_3 \psi_3$, which is normalized, $\langle v | v \rangle_\mu = 1$, and which is in the null space of $\tilde{\mathcal{T}}$, $\tilde{\mathcal{T}} \circ v = 0$.}\footnote{\DIFdel{To be concrete, set $a_1 = 0$, and choose $a_2$ and $a_3$ to satisfy $a_2 \langle \psi_2 | q \rangle_\mu  = - a_3 \langle \psi_3 | q \rangle_\mu$ and $a_2^2 + a_3^2 = 1$.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{Since $E[q]$ is the maximum of $E_{\mu_0}[q]$ over all possible $\mu_0$, it also must be greater than the error incurred for this particular starting distribution, $\mu_0=v$. Thus,
}\begin{align*}\DIFdel{
E}[\DIFdel{q}] &\DIFdel{\geq || (\mathcal{T}(t) - \tilde{\mathcal{T}}(t)) \circ v ||_\mu^2 }\\
&\DIFdel{= || \mathcal{T}(t) \circ v ||_\mu^2 }\\
&\DIFdel{= \sum_{i=1}^3 \lambda_i^{2t} a_i^2 }\\
&\DIFdel{\geq \lambda_3^{2t},
}\end{align*}
%DIFAUXCMD
\DIFdel{where the third line only includes a sum up to $i=3$ because, by construction, $v$ is in the span of the first three eigenfunctions. The final line follows because of the ordering of the eigenvalues and the normalization of $v$, implying $\sum_{i=1}^3 a_i^2 = 1$.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Interpreting this inequality, we see that the worst-case prediction error for any }\emph{\DIFdel{ansatz}} %DIFAUXCMD
\DIFdel{reaction coordinate, $q$, is always greater than or equal to $\lambda_3^{2t}$. Furthermore, for the particular choice $q=\psi_2$ and $f(t/\tau)=\lambda_2^t$, the equality is achieved, $E[\psi_2] = \lambda_3^{2t}$.}\footnote{\DIFdel{To demonstrate that $E[\psi_2] = \lambda_3^{2t}$, note that for this choice of $q$ and $f(t/\tau)$, $\tilde{\mathcal{T}}(t)$ is equal to the sum of the first two terms in the spectral decomposition of $\mathcal{T}(t)$. The squared spectral norm of the difference between the two operators is the square of the largest eigenvalue of the difference operator. The first two eigenpairs having been subtracted out, the square of the largest remaining eigenvalue is $\lambda_3^{2t}$.}} %DIFAUXCMD
\addtocounter{footnote}{-1}%DIFAUXCMD
\DIFdel{If we define $\tau \equiv -1/\ln \lambda_2$, we see also }\DIFdelend \DIFaddbegin \DIFadd{It follows, and is proven in Appendix~\ref{appendix:error}, }\DIFaddend that \DIFaddbegin \DIFadd{$\psi_2 = \min_q E[q]$ and that }\DIFaddend $f(t/\tau)$ can be written as $f(t/\tau) = e^{-t/\tau}$. \DIFdelbegin \DIFdel{Therefore}\DIFdelend \DIFaddbegin \DIFadd{Because it is the minimizer of $E[q]$}\DIFaddend , $\psi_2$ is the natural reaction coordinate\DIFdelbegin \DIFdel{, the minimizer of $E[q]$}\DIFdelend .


\DIFdelbegin \DIFdel{The reader may recall that this argument is equivalent to the Eckart-Young Theorem on the optimal low-rank approximation of a matrix.\mbox{%DIFAUXCMD
\cite{eckart1936approximation}
}%DIFAUXCMD
For self-adjoint linear operators, the original result is by Schmidt.\mbox{%DIFAUXCMD
\cite{schmidt1907zur}
}%DIFAUXCMD
See Courant and Hilbert (pp. 161),\mbox{%DIFAUXCMD
\cite{courant2008methods}
}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\citet{micchelli1971some}
}%DIFAUXCMD
for further details.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \subsection{Continuous-time Markov processes}
...
Markov state models (MSMs) and time-structure \DIFdelbegin \DIFdel{independent components }\DIFdelend \DIFaddbegin \DIFadd{based independent component }\DIFaddend analysis (tICA) are two widely used approximators for $\psi_2$ that can be parameterized directly from molecular dynamics trajectories.\cite{prinz2011markov, shukla2015markov, schwantes2013improvements, perez2013identification} Other popular estimators include diffusion maps and kernel tICA.\cite{nadler2006diffusion, coifman2008diffusion, rohrdanz2011determination, shwantes2015modeling, kim2015systematic}
...
\begin{align}
    C_{ij}(t) &= \langle \chi_i | \mathcal{T}(t) \circ \chi_j  \rangle_\mu = \mathbb{E}\left[ \chi_i(X_t) \cdot \chi\DIFdelbegin \DIFdel{_i}\DIFdelend \DIFaddbegin \DIFadd{_j}\DIFaddend (X_0) \right] \\
...
The tICA method has one obvious drawback: the solution, our approximate natural reaction coordinate, is a linear combination of all $m$ basis functions, and the loadings are typically non-zero. This makes the solutions difficult to interpret in a mechanistic manner, because hundreds or thousands of different interatomic distances and/or torsion angles, for example, \DIFdelbegin \DIFdel{can be }\DIFdelend \DIFaddbegin \DIFadd{have been }\DIFaddend combined together into a single collective variable. Because an important property of reaction coordinates is their role in facilitating physical interpretation of the underlying molecular system, we consider it desirable to reduce the number of explicitly used variables.
...
In this section, we introduce a new \emph{sparse} approximator for $\psi_2$. The solution will share the same form as the tICA approximation, \DIFdelbegin \DIFdel{$q = \sum_{i} a_i \chi_i$}\DIFdelend \DIFaddbegin \DIFadd{$q(x) = \sum_{i} a_i \chi_i(x)$}\DIFaddend , except that the vast majority of the expansion coefficients, $a_i$, will be zero. \DIFaddbegin \DIFadd{This method naturally extends to sparse approximators for each of the other leading eigenfunctions, $\psi_3, \ldots, \psi_k$.
}\DIFaddend 
...
\begin{align}
    ||\mathbf{x}||_0 = \sum_{i=1}\DIFdelbegin \DIFdel{^p }\DIFdelend \DIFaddbegin \DIFadd{^m }\DIFaddend 1_{\{|x_i| \neq 0\}} = \lim_{\epsilon \rightarrow 0} \sum_{i=1}\DIFdelbegin \DIFdel{^p }\DIFdelend \DIFaddbegin \DIFadd{^m }\DIFaddend \frac{\log(1+|x_i|/\epsilon)}{\log(1+1/\epsilon)}.
\end{align}
...
\label{eq:sticaopt}
\begin{split}
\operatornamewithlimits{maximize}_\mathbf{x} \hspace{2em}& \mathbf{x}^T \mathbf{C} \mathbf{x} - \rho \sum_{i=1}\DIFdelbegin \DIFdel{^n }\DIFdelend \DIFaddbegin \DIFadd{^m }\DIFaddend \frac{\log(1+|x_i|/\epsilon)}{\log(1+\epsilon)} \\
...
\begin{algorithm}[H]
\begin{algorithmic}
%DIF > \Require $\mathbf{C} \in \mathbb{S}^n, \mathbf{\Sigma} \in \mathbb{S}_{++}^n, \rho>0, \epsilon>0$
\Require \DIFdelbegin \DIFdel{$\mathbf{C} \in \mathbb{S}^n, \mathbf{\Sigma} \in \mathbb{S}_{++}^n, \rho>0, \epsilon>0$
}\DIFdelend \DIFaddbegin \DIFadd{$\mathbf{C}$ is a $n \times n$ real symmetric matrix, $\mathbf{\Sigma}$ is a $n \times n$ positive definite matrix, $\rho>0$, $\epsilon>0$
}\Ensure \DIFadd{$\mathbf{D}(\mathbf{w}^{(l)})$ be a diagonal matrix with $(w_1^{(l)},\dots,w_n^{(l)})$ as its principal diagonal, $\lambda_{min}(\mathbf{C})$ be the smallest eigenvalue of the matrix $\mathbf{C}$
}\DIFaddend \State Choose $\tau > \max(0, -\lambda_{min}(\mathbf{C}))$, $\mathbf{x}^{(0)} \in \{\mathbf{x} : \mathbf{x}^T \mathbf{\Sigma} \mathbf{x} \leq 1 \}$
...
\mathbf{z}^{(k+1)} &= \argmin_\mathbf{z} \left(g(\mathbf{z}) + \frac{\varrho}{2}||\mathbf{x}^{(k+1)} - \mathbf{z} \DIFdelbegin \DIFdel{^{(k)} }\DIFdelend + \mathbf{u}^{(k)}||^2_2 \right) \label{eq:zopt} \\
...
Although this objective function is not differentiable, it is a simple application of subdifferential calculus to compute a \DIFdelbegin \DIFdel{simple }\DIFdelend closed-form expression for the minimizer \DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{(see Ref.~}\onlinecite{Rockafellar_Book70}\DIFadd{, \textsection 23 for background). }\DIFaddend The explicit solution is
\begin{align}
...
Alternatively, when the primary goal is to generate physically interpretable reaction coordinates, the modeller may choose the value of $\rho$ to bring the number of non-zero loadings down to a pre-specified number that is amenable to interpretation. When employing this strategy, we recommend that modellers watch the value of the \DIFdelbegin \DIFdel{pseudoeiegenvalue }\DIFdelend \DIFaddbegin \DIFadd{pseudoeigenvalue }\DIFaddend (Rayleigh quotient), $\hat{\lambda}=\mathbf{x}^T\mathbf{C}\mathbf{x} / \mathbf{x}^T \mathbf{\Sigma}\mathbf{x}$. It should decrease slightly with increasing $\rho$, but dramatic drops in $\hat{\lambda}$ may indicate over-regularization.
...
\section{Examples}\DIFaddbegin \label{sec:examples}
\DIFaddend \subsection{Torsional reaction coordinate}
...
\begin{figure}
    \includegraphics[width=0.5\textwidth]{arg42-phi-angle-crop.pdf}
...
%DIF >  beh
\DIFaddbegin \subsection{\DIFadd{Folding of a three-helix bundle}}

\begin{figure*}
\centering
\includegraphics[width=0.85\textwidth]{a3d.pdf}
\onecolumngrid
\caption{\label{fig:a3d}\DIFaddFL{(a) Superimposing a plot of the slowest sparse tICA solution, $\psi_2$, and the RMSD of the protein conformation to the folded structure shows that the inter-residue contact distance isolated by $\psi_2$ does not correspond to the folding of $\alpha$3D. (b) Superimposing a plot of the next sparse tICA solution, $\psi_3$, and the RMSD of the protein conformation to the folded structure shows that either inter-residue contact distance implicated in $\psi_3$ serves as a suitable reaction coordinate for folding. (c) The folded state of $\alpha$3D illustrating the residue pairs defining the contact distances retained in $\psi_2$ (violet) and $\psi_3$ (blue and cyan). Only the first half of the dataset is shown.}}
\end{figure*}

\DIFadd{Next we use the sparse tICA algorithm to elucidate a specific process; in this case, the folding of $\alpha$3D, a 73-residue three-helix bundle.\mbox{%DIFAUXCMD
\cite{Walsh_PNAS99, Kubelka_COSB04}
}%DIFAUXCMD
We analyzed the $\alpha$-carbon trace of a 707 $\mu$s molecular dynamics dataset for $\alpha$3D generated by \mbox{%DIFAUXCMD
\citet{LindorffLarsen_Science11}
}%DIFAUXCMD
. The protein folds and unfolds 12 times over the course of the simulations. We extracted inter-residue $\alpha$-carbon distances for all pairs separated by least two residues from each frame for a total of 2485 distances. From these distances, we fit a sparse tICA model ($\rho$=0.5).
}
...
%DIF >  beh
\DIFaddbegin \DIFadd{When applying sparse tICA to folding simulations of $\alpha$3D, we find that a nondominant reaction coordinate, $\psi_3$, serves as a reaction coordinate for folding while the dominant reaction coordinate, $\psi_2$, instead captures a seemingly unrelated, rare contraction of the distance between two residues close in the protein sequence. This example highlights that the desired reaction coordinate many not be the first (i.e.~slowest) solution to the algorithm. Furthermore, when the process corresponding to the dominant reaction coordinate seems unrelated to the process of interest, it may indicate that the system dynamics have been insufficently sampled, or motivate inspection of the force field parameters related to the features controlling $\psi_2$.
}

\DIFaddend We anticipate that this method will be useful for the analysis of today's large molecular dynamics data sets. An implementation of this estimator is available in the MSMBuilder software package at \url{http://msmbuilder.org/} under the GNU Lesser General Public License.

\section*{Acknowledgments}
The authors thank \DIFdelbegin \DIFdel{Brooke Husic and }\DIFdelend Thomas J. Lane for helpful discussions made during the preparation of this manuscript, Ariana Peck and Carlos X. Hern\'{a}ndez for invaluable copy editing, and the National Institutes of Health under Nos\DIFaddbegin \DIFadd{. }\DIFaddend NIH R01-GM62868 for funding. We graciously acknowledge D.E. Shaw Research for providing access to the BPTI trajectory data set.
%DIF > The authors thank Brooke Husic and Thomas J. Lane for helpful discussions made during the preparation of this manuscript,

\appendix
\DIFaddbegin \section{\DIFadd{Analysis of the error functional}}
\label{appendix:error}

\DIFadd{To prove by why $\psi_2 = \min_q E[q]$, (Eqn.~\ref{eq:error_functional}), observe that for any $q$, there exists a function $v(x)$ in the span of the first three eigenfunctions of $\mathcal{T}$, $v=a_1 \psi_1+ a_2 \psi_2 + a_3 \psi_3$, which is normalized, $\langle v | v \rangle_\mu = 1$, and which is in the null space of $\tilde{\mathcal{T}}$, $\tilde{\mathcal{T}} \circ v = 0$.}\footnote{\DIFadd{To be concrete, set $a_1 = 0$, and choose $a_2$ and $a_3$ to satisfy $a_2 \langle \psi_2 | q \rangle_\mu  = - a_3 \langle \psi_3 | q \rangle_\mu$ and $a_2^2 + a_3^2 = 1$.}} \DIFadd{Since $E[q]$ is the maximum of $E_{\mu_0}[q]$ over all possible $\mu_0$, it also must be greater than the error incurred for this particular starting distribution, $\mu_0=v$. Thus,
}\begin{align}\DIFadd{
E}[\DIFadd{q}] &\DIFadd{\geq || (\mathcal{T}(t) - \tilde{\mathcal{T}}(t)) \circ v ||_\mu^2 }\\
&\DIFadd{= || \mathcal{T}(t) \circ v ||_\mu^2 }\\
&\DIFadd{= \sum_{i=1}^3 \lambda_i^{2t} a_i^2 }\\
&\DIFadd{\geq \lambda_3^{2t},
}\end{align}
\DIFadd{where the third line only includes a sum up to $i=3$ because, by construction, $v$ is in the span of the first three eigenfunctions. The final line follows because of the ordering of the eigenvalues and the normalization of $v$, implying $\sum_{i=1}^3 a_i^2 = 1$.
}

\DIFadd{Interpreting this inequality, we see that the worst-case prediction error for any }\emph{\DIFadd{ansatz}} \DIFadd{reaction coordinate, $q$, is always greater than or equal to $\lambda_3^{2t}$. Furthermore, for the particular choice $q=\psi_2$ and $f(t/\tau)=\lambda_2^t$, the equality is achieved, $E[\psi_2] = \lambda_3^{2t}$.}\footnote{\DIFadd{To demonstrate that $E[\psi_2] = \lambda_3^{2t}$, note that for this choice of $q$ and $f(t/\tau)$, $\tilde{\mathcal{T}}(t)$ is equal to the sum of the first two terms in the spectral decomposition of $\mathcal{T}(t)$. The squared spectral norm of the difference between the two operators is the square of the largest eigenvalue of the difference operator. The first two eigenpairs having been subtracted out, the square of the largest remaining eigenvalue is $\lambda_3^{2t}$.}} \DIFadd{If we define $\tau \equiv -1/\ln \lambda_2$, $f(t/\tau)$ can be written as $f(t/\tau) = e^{-t/\tau}$. Therefore $\psi_2$ is the natural reaction coordinate, the minimizer of $E[q]$.
}

\DIFadd{The reader may recall that this argument is equivalent to the Eckart-Young Theorem on the optimal low-rank approximation of a matrix.\mbox{%DIFAUXCMD
\cite{eckart1936approximation}
}%DIFAUXCMD
For self-adjoint linear operators, the original result is by Schmidt.\mbox{%DIFAUXCMD
\cite{schmidt1907zur}
...
\DIFaddend \section{Covariance matrix estimation}
...
\section*{References}
\DIFdelbegin %DIFDELCMD < \bibliography{main.bbl}
%DIFDELCMD < %%%
\DIFdelend %DIF > \bibliography{main.bbl}
\DIFaddbegin \bibliography{references}
\DIFaddend 
