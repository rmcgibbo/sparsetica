...
\end{align}
for some non-random function, $f$, that is independent of $x$.
{\color{red}Later on, we will show that $f(t/\tau)=e^{-t/\tau}$, i.e.~the autocorrelation function of the \textit{ansatz} reaction coordinate $q$.}
%Later on, we will show that $f$ is necessarily an exponential, $f(t/\tau)=e^{-t/\tau}$.
For diffusion on a double well potential, a diagrammatic example of the family of predictions, $\tilde{p}_t$, that can be made given a particular choice of $q$ is shown in \cref{fig:eq1}. \hl{[TODO: Reviewer 1 thinks (1) must be motivated more clearly, and discusses this extensively in the attached comments. My autocorrelation comment and softening of the original text addresses just some of this, and I'm not sure if it's sufficient.]}
...
The functional $E[q]$ thus measures how well the measurement of an arbitrary collective variable can be used to predict the future state of the system. We define the \emph{natural reaction coordinate}, $q^*$, as the minimizer of $E[q]$. It is, in this sense, the collective variable which is maximally informative about the system's dynamics. \hl{[TODO: Reviewer 1 suggests that the maximum of $E[q]$ could be infinite; this is discussed in Reviewer 1 attachment. It seems that in order to maintain the math of the rest of the paper, this exception should be acknowledged diplomatically (if we agree with it). Furthermore, R1 doesn't see the connection between eqns. (4) and (18).]}
...
{\color{red}Another reaction coordinate definition worth mentioning is the second eigenfunction (or the subset of leading eigenfunctions) of a diffusion process.\cite{nadler2006diffusion} In this formulation, a map is created from sampled points in phase space by utilizing a geometric distance metric, where points that are close geometrically are expected to correspond to kinetically similar conformations. We first note that the diffusion map formulation requires infinite sampling in order to ensure that the (geometric) diffusion distance corresponds to the manifold on which the system's underlying dynaimcs occur. Second, it is noteworthy that results ascertained from diffusion maps are invariant to the time-indexing of trajectory frames; in other words, the duration of the path between any two conformations is not utilized for analysis. As a result, diffusion maps do not provide a straightforward way to estimate the timescale for a given eigenvector. In contrast, the natural reaction coordinate defined in this work yields a mathematical relationship between a dynamical process's eigenvector and its associated timescale, and thus directly provides kinetic information about the process.}
...
In this section, we demonstrate that the {\color{red}(dominant)} natural reaction coordinate, as defined by the minimizer of \cref{eq:defn}, is the second leading eigenfunction of an integral operator associated with a system's Markovian dynamics in $\Omega$. For simplicity, we work here with a discrete-time Markov chain, $\{X_0, X_1, X_2, \ldots \}$, such as a typical all-atom MD simulation with a finite time step integrator, assuming only that the prediction interval $t$ is greater than 1 step (typically on the order of 2 fs). Afterwards, we note why the same results apply if the underlying dynamics are a continuous-time Markov process, and discuss the natural generalization to multiple reaction coordinates.
...
\end{align}
where the maximum is understood to be taken over properly normalized $u_0$, $||u_0||_\mu = 1$. \hl{[TODO: R1 thinks the problem statement of (18) is finding the best approximation in operator norm to the transfer operator from a rank two operator, and that this has a known solution, i.e.~truncating the spectral expansion after the second term. R1 also doesn't see how this relates to Eqn.~(4).]}
...
{\color{red}As an example, sufficiently long molecular dynamics simulations of protein folding might yield a dominant reaction coordinate that is associated with a \textit{trans-cis} transition as opposed to with the folding process.\cite{Banushkina_JCP15} In this case, the reaction coordinate of interest is expected to be the third transfer operator eigenfunction, $\psi_3$. In general, systems containing} $k$ metastable states will have $k-1$ eigenfunctions whose associated eigenvalues are close to one, separated from the remaining eigenvalues by a so-called spectral gap.\cite{prinz2011markov}
...
In this section, we introduce a new \emph{sparse} approximator for $\psi_2$. {\color{red}We note that this method extends to sparse approximators for each of the leading eigenfunctions, $\psi_2, \ldots, \psi_k$, but restrict the present discussion to $\psi_2$ for succinctness.} The solution will share the same form as the tICA approximation, $q = \sum_{i} a_i \chi_i$, except that the vast majority of the expansion coefficients, $a_i$, will be zero.
...
\end{align}
where $\mathbf{A}=\mathbf{B}=\mathbf{I}_n$, and $\mathbf{c}=0$. The ADMM algorithm, in so-called scaled form, consists of the following iterations. \hl{[TODO: Reviewer 1 says that in Eqn. 36 (pdf)/eq:zopt (tex), $\mathbf{z}^{(k)}$ should be $\mathbf{z}$.]}
\begin{align}
...
\begin{figure*}
\centering
\includegraphics[width=\textwidth]{a3d.pdf}
\onecolumngrid
\caption{\label{fig:a3d} caption} %todo
\end{figure*}

{\color{red} Next we use the sparse tICA algorithm to elucidate a specific process; in this case, the folding of $\alpha$3D, a 73-residue three-helix bundle.\cite{Walsh_PNAS99, Kubelka_COSB04} We analyzed the $\alpha$-carbon trace of a 707 $\mu$s molecular dynamics dataset for $\alpha$3D generated by \citet{LindorffLarsen_Science11}. The protein folds and unfolds 12 times over the course of the simulations. We extracted all inter-residue $\alpha$-carbon distances (for all pairs separated by least two residues) from each frame for a total of 2485 distances. From these distances, we fit a sparse tICA model ($\rho$=0.5). }

{\color{red} The first solution depends on just one feature: the distance between GLY 49 and GLU 52. These residues are close in the sequence and typically remain separated by about 9 $\AA$; however, they occasionally are found within 6 $\AA$ of each other. A plot of the GLY 49--GLU 52 distance superimposed over a plot of the conformation's root-mean-square deviation (RMSD) shows no obvious relationship between this contact distance and the folding process (Fig.~\ref{fig:a3d}a). However, trajectory events characterized by the shortening of the GLY 49--GLU 52 distance occur more rarely than folding events, and thus this contraction is the slowest process found by tICA. }

{\color{red} However, we want to utilize the sparse tICA algorithm specifically to investigate the folding process. The second solution isolates two residue contact pairs: GLU 32--ASP 65 and GLU 39--ARG 57. Both pairs contain residue contacts between the same two $\alpha$-helices. Fig.~\ref{fig:a3d}b shows a plot of the two contact distances comprising the second tIC superimposed with the conformation's RMSD. It is clear that both the GLU 32--ASP 65 and GLU 39--ARG 57 distances are closely related to whether the protein is folded or unfolded, and thus either distance is expected to be suitable to serve as a reaction coordinate for the folding of $\alpha$3d. Further analysis might be performed to determine whether the aligning of the two helices involved in the second tIC is the rate-limiting step of the folding proess. }
...
% beh
{\color{red} When applying sparse tICA to folding simulations of $\alpha$3D, we find that the second tIC serves as a reaction coordinate for folding while the first tIC captures a seemingly unrelated, rare contraction of the distance between two residues close in the protein sequence. This example highlights that the desired reaction coordinate many not be the first (i.e.~slowest) solution to the algorithm. Furthermore, when the process corresponding to the first solution seems unrelated to the slowest process, it may indicate that the system dynamics have been insufficently sampled, or motivate inspection of the force field parameters related to the features controlling that process. }
